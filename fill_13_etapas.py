...
# Copiar el archivo para no depender de si Excel lo tiene abierto
import shutil
temp = os.path.join(os.getcwd(), 'temp_completo.xlsx')
shutil.copy2(path, temp)
wb = openpyxl.load_workbook(temp)

# Crear nueva hoja "13 etapas"
if '13 etapas' in wb.sheetnames:
    del wb['13 etapas']
ws = wb.create_sheet('13 etapas')

# Headers
headers = ['N', 'VALOR', 'Etapa', 'Proceso',
           'Descripci\u00f3n de la Etapa (Valor Entregado)',
...
data = [
    ...
    {
        'n': 12,
        'etapa': 'Seguimiento, Evaluaci\u00f3n y Cierre de Ciclo',
        'proceso': 'Comunicaci\u00f3n post-NC, seguimiento del CC, documentaci\u00f3n y lecciones aprendidas',
        'desc_etapa': (
            'Informar a la organizaci\u00f3n los resultados, verificar el cumplimiento sostenido '
            'del CC, preservar toda la documentaci\u00f3n generada y extraer aprendizajes '
            'para preparar el pr\u00f3ximo ciclo de negociaciones.'
        ),
        'responsable': 'Relaciones Sindicales, Comunicaciones Internas, Operaci\u00f3n, Compensaciones, Gerencia de Personas, ECO',
        'desc_proceso': (
            ...
            'y se monitorean indicadores de clima laboral. '
            'Se realiza an\u00e1lisis retrospectivo (tiempos, costos, d\u00edas de huelga, impacto operacional), '
            'se documentan lecciones aprendidas y se archivan en SharePoint por sindicato y a\u00f1o: '
            'n\u00f3minas, SSMM, estados de tiendas, CC firmados, finiquitos, actas y correspondencia con la DT. '
...
# Guardar en el mismo path
try:
    wb.save(path)
    print(f'\n\u2705 Guardado directamente en:\n   {path}', flush=True)
except PermissionError:
    # Si Excel lo tiene abierto, guardar al lado
    alt = path.replace('COMPLETO', 'COMPLETO v2')
    wb.save(alt)
    path = alt
    print(f'\n\u26a0 Original abierto, guardado como:\n   {alt}', flush=True)
print(f'   Nueva hoja: "13 etapas"', flush=True)
...
print(f'\n{len(data)} etapas escritas (N=0 a N=12)', flush=True)
for d in data:
